...
CREATE TABLE IF NOT EXISTS metadata (key TEXT PRIMARY KEY, value BLOB NOT NULL);`); err != nil {
	...
	case 6:
		_, err := tx.Exec(`CREATE TABLE IF NOT EXISTS deployment_attempts (
			id TEXT PRIMARY KEY CHECK(typeof(id)='text' AND length(id)=32 AND id NOT GLOB '*[^0-9a-f]*'),
			job_id TEXT NOT NULL CHECK(typeof(job_id)='text' AND length(job_id)=32 AND job_id NOT GLOB '*[^0-9a-f]*'),
			attempt_id TEXT NOT NULL CHECK(typeof(attempt_id)='text' AND length(attempt_id)=32 AND attempt_id NOT GLOB '*[^0-9a-f]*'),
			...
			expected_tree_sha TEXT NOT NULL CHECK(typeof(expected_tree_sha)='text' AND length(expected_tree_sha)=40 AND expected_tree_sha NOT GLOB '*[^0-9a-f]*'),
			...
			phase TEXT NOT NULL CHECK(typeof(phase)='text' AND phase IN ('pending','preparing','activating','healthchecking','awaiting_acceptance','failed')),
