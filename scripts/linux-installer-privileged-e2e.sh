...
for tool in cmp getent grep id journalctl mkfifo mktemp readlink rm sha256sum stat systemctl tar tr uname python3; do
  command -v "$tool" >/dev/null 2>&1 || { echo "missing required tool: $tool" >&2; exit 1; }
done
for path in /usr/bin/cmp /usr/bin/getent /usr/bin/grep /usr/bin/id /usr/bin/journalctl /usr/bin/mkfifo /usr/bin/mktemp /usr/bin/readlink /usr/bin/rm /usr/bin/sha256sum /usr/bin/stat /usr/bin/systemctl /usr/bin/tar /usr/bin/tr /usr/bin/python3; do
