...
"$ROOT/bin/forge-gate" -addr 127.0.0.1:18081 -db "$RUN/forge.db" -worker-id coding-worker -worker-token synthetic-token >"$RUN/gate.log" 2>&1 &
GATE_PID=$!
i=0
until curl -sS -o /dev/null http://127.0.0.1:18081/v1/workers/coding-worker 2>/dev/null; do
  i=$((i+1)); [ "$i" -lt 50 ] || { echo "gate failed to become ready"; exit 1; }; sleep 0.1
done
CODEX_BIN=${CODEX_BIN:-codex} "$ROOT/bin/forge-worker" -gate ws://127.0.0.1:18081 -id coding-worker -token synthetic-token -plugin "$ROOT/bin/forge-codex-plugin" >"$RUN/worker.log" 2>&1 &
WORKER_PID=$!
i=0
until curl -fsS http://127.0.0.1:18081/v1/workers/coding-worker 2>/dev/null | python3 -c 'import json,sys; assert json.load(sys.stdin)["connected"]' 2>/dev/null; do
  i=$((i+1)); [ "$i" -lt 50 ] || { echo "worker failed to connect"; exit 1; }; sleep 0.1
done

SUBMIT=$(python3 - "$REPO" "$BASE" <<'PY' | curl -fsS -X POST http://127.0.0.1:18081/v1/jobs -H 'Content-Type: application/json' --data-binary @-
import json,sys
...
EVENTS=$(curl -fsS "http://127.0.0.1:18081/v1/jobs/$JOB_ID/events")
